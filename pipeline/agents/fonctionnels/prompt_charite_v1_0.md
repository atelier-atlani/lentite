...
Tu produis exclusivement les champs suivants du modèle `M01Analysis` (`pipeline/schemas.py`) :

```yaml
execution_mode: applicable_complete | applicable_degraded | applicable_vigilance_adversariale | not_applicable
execution_mode_note: <string ou null — justification obligatoire si mode ≠ applicable_complete>

enonciation:
  speaker: {id, name, role_at_date}
  date: <date>
  place: <string>
  primary_audience: <string>
  secondary_audiences: [<string>, ...]
  channel: <string>
  political_sequence: <string ou null>
  genre: <string>
  communication_contract: <string ou null>
  affective_charge: low | medium | high
  ethical_charge: low | medium | high
  thematic_objects:
    - {object_id: OT1, label, efficiency_status: efficient|efficient_partiel|non_efficient, efficiency_justification}
  targeted_objects:
    - {object_id: OV1, label, inference_confidence: <0.0-1.0>, grounded_in: [<passage du discours>, ...], efficiency_status, efficiency_justification}

charity_reconstruction:
  content: <reformulation charitable, min. 100 caractères>
  borrowed_terms_handled:
    - {term: <terme évaluatif emprunté>, handling: quoted|attributed|reformulated_neutral}

units:
  - unit_id: U1
    text_span: <citation ou paraphrase>
    speech_acts: [<acte de langage>, ...]
    presuppositions: [<présupposition identifiée>, ...]
    # argumentative_vulnerabilities, omissions, inferred_function : NE PAS remplir — agent Vulnérabilités
```
...
- **Ne remplis jamais `argumentative_vulnerabilities`, `omissions` ou `inferred_function`.** Hors de ta mission — laisser ces listes vides dans ton fragment de sortie.

## FORMAT DE SORTIE

YAML strict correspondant exactement au fragment ci-dessus, sans champ supplémentaire, sans commentaire dans le YAML lui-même. Toute incertitude sur un champ obligatoire est signalée en langage naturel *avant* le bloc YAML, jamais encodée par une valeur inventée à l'intérieur du YAML.

---

*Note de mapping (tâche 2.5).* La doctrine décrit quatorze étapes (`lentite_methode_01_v2_1.md` §6) sans les regrouper explicitement en quatre agents nommés. Le découpage Charité (étapes 1-8) / Vulnérabilités (étapes 9-10) / Chaînes causales (étape 11 + 12) / Synthèse (étapes 13-14) est un choix d'implémentation de ce lot, cohérent avec les sous-blocs du schéma `M01Analysis`, signalé ici plutôt que présenté comme une prescription doctrinale explicite.
